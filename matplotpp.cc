/****************************************************************************
Copyright (c) 2011-2016 Yuichi Katori All Rights Reserved
...
dvec Axes::make_tick(double min,double max){
  ...
  if(max>min){ 
    ...
    while(max>=min0+dg*i){
      t=min0+dg*i;
      //if(min+0.5*dg<t && t<min+dg){tick.push_back(min);}
      if(min<=t){ tick.push_back(t); j++; } 
      i+=1;
    } 
  }
  if(max<min){ 
    i=-2; while(max<=min0-dg*i){ 
...
void MatPlot::display(){
  ...
  while(!is_done){
    ...
    if(mode==1){// Configuration of objects
      ...
      if(vColorbar.size()!=iColorbar ){ mode_next=0; if(is_debug1){cout<<"not match in Colorbar"<<endl;}}

      //if(mode_next==0){printf("asdf\n");}
...
void MatPlot::SPrintx(char *s,double x){
  int H=1+(int)log10(x);
  double r=fmod(fabs(x),1.0);
  int X=(int)fabs(x*1000000);
  int Y=1000000;
  int L=0;
  for(int i=1;i<=6;i++){ 
    //printf("X%Y %d\n",X/Y); 
  ...
  //printf("%15.8f H=%2d L=%d ",x,H,L);
  if(H>= 3){
    sprintf(s,"%.0f ",x);
  }else{
    if(L==0){sprintf(s,"%.1f ",x);}
    if(L==1){sprintf(s,"%.2f ",x);}
    if(L==2){sprintf(s,"%.3f ",x);}
    if(L==3){sprintf(s,"%.4f ",x);}
    if(L==4){sprintf(s,"%.5f ",x);}
    if(L==5){sprintf(s,"%.6f ",x);}
    if(L>=6){sprintf(s,"%.7f ",x);}
  }
}
...
void MatPlot::xaxis(double xMin,double xMax){//not in Matlab
  if(xMin!=xMax){
    ca->XLim[0]=xMin;
    ca->XLim[1]=xMax;
    ca->XLimMode=1;
...
void MatPlot::yaxis(double yMin,double yMax){//not in Matlab
  if(yMin!=yMax){
    ca->YLim[0]=yMin;
    ca->YLim[1]=yMax;		
    ca->YLimMode=1;
...
void MatPlot::caxis(double cMin,double cMax){
  ca->CLim[0]=cMin;
  ca->CLim[1]=cMax;
  //ca->XLimMode=1;
}
...
void MatPlot::mouse_capture(float *xmouse,float *ymouse){
  ca->Mouse=1;
  *xmouse=(float)ca->XMouse;
  *ymouse=(float)ca->YMouse;
}
...
void MatPlot::display_axes_colorbar(){
  ...
  if(ca->Box){
    ...
    // z tick 
    //return ca->Position[1] +ca->Position[3]*( (y-ca->YLim[0])/(ca->YLim[1]-ca->YLim[0]) );
    for(int i=0;i<ca->CTick.size();++i){
      glBegin(GL_LINE_STRIP);
      //glVertex2d( l+w,       cty2(ca->CTick[i]) );
      //glVertex2d( l+w+0.01,  cty2(ca->CTick[i]) );
      glVertex2d( l+w,       b+h*((ca->CTick[i] - ca->CLim[0])/(ca->CLim[1] - ca->CLim[0])) );
      glVertex2d( l+w+0.01,  b+h*((ca->CTick[i] - ca->CLim[0])/(ca->CLim[1] - ca->CLim[0])) );
      glEnd();
    }
    // z tick number
    for(int i=0;i<ca->CTick.size();++i){
      //sprintf(ctmp,"%4.1f",ca->CTick[i]);
      SPrintx(ctmp,ca->CTick[i]);
      ptext( l+w+0.01,b+h*((ca->CTick[i] - ca->CLim[0])/(ca->CLim[1] - ca->CLim[0])),ctmp );
    }
  }//Box
...
/// create semilogx, semilogy, loglog
...
void MatPlot::display_line(){
  ...
  float xx,yy,zz;// transformed coordination 
  float r;//marker size
  float rx,ry;
  ...
  if(ca->View==1){
	...
	if(cl->Marker=="+"){//+
	  glBegin(GL_LINE_STRIP); 
	  glVertex3d(xx-rx,yy,zz);
	  glVertex3d(xx+rx,yy,zz);
	  glEnd();
	  glBegin(GL_LINE_STRIP); 
	  glVertex3d(xx,yy-ry,zz);
	  glVertex3d(xx,yy+ry,zz);
	  glEnd();
	}
      }
...
void MatPlot::surface_config(){
  ...
  cs->CLim[0]=Fmin(cs->CDataIndex);
  cs->CLim[1]=Fmax(cs->CDataIndex);
  //if(ca->CLim[0]==ca->CLim[1]){
  if(ca->CLimMode=="auto"){
    ca->CLim[0]=cs->CLim[0];
    ca->CLim[1]=cs->CLim[1];
    //if(ca->CLim[0]==ca->CLim[1]){ ca->CLim[0]=0; ca->CLim[1]=1; }
  }
...
tcolor MatPlot::colorspec2rgb(string c){
  tcolor tc; tc.clear();
  float r,g,b;
  sscanf(c.c_str(),"[%lf %lf %lf]",&r,&g,&b);
  tc.push_back(r);
  tc.push_back(g);
  tc.push_back(b);
  return tc;
